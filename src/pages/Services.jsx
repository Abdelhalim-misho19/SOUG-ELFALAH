...
import { price_range_service, query_services } from '../store/reducers/homeReducer';
import ServiceProducts from '../components/services/ServiceProducts'; // New component for services

const Services = () => {
    const dispatch = useDispatch();
    const { services, serviceCategories, priceRange, latest_service, totalService, parPage } = useSelector(state => state.home);

    useEffect(() => { 
        dispatch(price_range_service());
    }, []);
                ...
                <div className='absolute left-0 top-0 w-full h-full bg-[#2422228a]'>
                    <div className='w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto'>
                        <div className='flex flex-col justify-center gap-1 items-center h-full w-full text-white'>
                            <h2 className='text-3xl font-bold'>Services Page</h2>
            ...
            <section className='py-16'>
                <div className='w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto'>
                    <div className={`md:block hidden ${!filter ? 'mb-6' : 'mb-0'}`}>
                        <button onClick={() => setFilter(!filter)} className='text-center w-full py-2 px-3 bg-indigo-500 text-white'>Filter Services</button>
                    </div>
                    ...
                    <div className='w-full flex flex-wrap'>
                        <div className={`w-3/12 md-lg:w-4/12 md:w-full pr-8 ${filter ? 'md:h-0 md:overflow-hidden md:mb-6' : 'md:h-auto md:overflow-auto md:mb-0'}`}>
                            <h2 className='text-3xl font-bold mb-3 text-slate-600'>Service Categories</h2>
                            <div className='py-2'>
                                {serviceCategories.map((c, i) => (
                                    <div key={i} className='flex justify-start items-center gap-2 py-1'>
                                        <input checked={category === c.name} onChange={(e) => queryCategory(e, c.name)} type="checkbox" id={c.name} />
                            ...
                            <div className='py-2 flex flex-col gap-5'>
                                ...
                                <Range
                                    ...
                                    renderTrack={({ props, children }) => (
                                        <div {...props} className='w-full h-[6px] bg-slate-200 rounded-full cursor-pointer'>{children}</div>
                                    )}
                                    renderThumb={({ props }) => (
                            ...
                            <div className='py-3 flex flex-col gap-4'>
                                ...
                                <div className='flex flex-col gap-3'>
                                    {[5, 4, 3, 2, 1, 0].map((r) => (
                                        <div key={r} onClick={() => r === 0 ? resetRating() : setRating(r)} className='text-orange-500 flex justify-start items-start gap-2 text-xl cursor-pointer'>
                        ...
                        <div className='w-9/12 md-lg:w-8/12 md:w-full'>
                            <div className='pl-8 md:pl-0'>
                                <div className='py-4 bg-white mb-10 px-3 rounded-md flex justify-between items-start border'>
                                    <h2 className='text-lg font-medium text-slate-600'>({totalService}) Services</h2>
                                ...
                                <div className='pb-8'>
                                    <ServiceProducts services={services} styles={styles} />
                                </div>

                                <div>
                                    {totalService > parPage && (
                                        <Pagination pageNumber={pageNumber} setPageNumber={setPageNumber} totalItem={totalService} parPage={parPage} showItem={Math.floor(totalService / parPage)} />
                                    )}
                                </div>
                            </div>
                        </div>
